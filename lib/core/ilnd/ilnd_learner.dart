...
class IlndLearner {
  ...
  Future<void> learnFrom(String text) async {
    if (text.trim().length < 12) return; // çok kısa metinden öğrenecek bir şey yok
    try {
      final service = _ref.read(ilndServiceProvider);
      final memory = _ref.read(ilndMemoryProvider);
      final extracted = await service.extractMemory(text: text, known: memory);
